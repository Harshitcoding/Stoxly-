...
export const sendSignUpEmail = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        await step.run('send-welcome-email', async() => {
            ...
            const introText = (part && 'text' in part ? part.text : null) || "Thanks for joining stoxly. You now have the tool to track markets and make smarter moves"
       ...
       for (const {user, articles} of results) {
        try {
            ...
            userNewsSummaries.push({
                user: { id: user.email, name: user.name, email: user.email },
                newsContent
            })

        } catch (error) {
            console.log('Failed To summarize news for:', user.email);
            // Provide a fallback id (e.g., user.email as id if no id is available)
            userNewsSummaries.push({
                user: { id: user.email, name: user.name, email: user.email },
                newsContent: null
            });
